...
export async function listDocuments(closingId: string, actor: FileActor) {
  ...
  return { documents: docs.map(doc => dto(doc, actor)), canManage: actor.staff, uploadsEnabled: storageEnabled(),
...
export async function documentAction(closingId: string, actor: FileActor, raw: unknown) {
  ...
  if (input.action === 'download') {
    ...
    await prisma.closingDocumentEvent.create({ data: { documentId: doc.id, actorId: actor.id, kind: 'download_authorized', metadata: { revision: doc.revision } } })
