...
export async function sendEOIntroductionEmail(d: {
  ...
  const eo = d.officer
  const personalAccess = new URL(d.dashboardUrl).pathname.startsWith('/file-access/')
  const accessNote = personalAccess
    ? 'You can reply without creating an account. No password or account setup needed to view your file. This personal access link works once and expires after 24 hours. Please don’t forward it.'
