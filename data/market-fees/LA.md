...
**2 of 3 calculator-basis providers — still below 3-provider threshold** (prior: 1 (FNF national rate calculator — East Baton Rouge Parish, Grand Total $2,345.20)). See LA.json's newest `basis: "calculator"` entry for full itemized figures and methodology.
...
- **WFG National Title Insurance Company** (`rates.wfgnationaltitle.com`) — WORKING. East Baton Rouge County (LA's most-populous/standard-scenario county), Baton Rouge, LA, $500,000 purchase / $400,000 loan, SettlementStatementVersion `CD`. Result: Owner's Title Insurance Premium **$2,579.72**, premium-only. Loan Policy premium returned $0/null in every state tried this session, consistent with this tool's seller-net-sheet (seller-side) design — not pursued further as out of scope.

## Calculator harvest addendum (2026-08-09) — Old Republic's second tool crosses LA to 3 providers

**3 of 3 calculator-basis providers — calculator-quoted** (prior: 2 — FNF, WFG). See LA.json's newest `basis: "calculator"` entry for full itemized figures and methodology.

Old Republic's second calculator tool (`ortratecalculator.oldrepublictitle.com`, distinct from `ortconline.com/Web2`, whose footprint does not reach LA) had `Location=LA` logged as NoBot-blocked since the 2026-08-01 session. This session found the block has loosened (same discovery made in parallel for SC — see SC.md's addendum) and successfully drove the tool for LA's "PURCHASE/SALE" combined category (Purchase Price + Loan Amount fields both present without a separate category-selection step, unlike SC's form).

- **Old Republic Title Insurance Company** (`ortratecalculator.oldrepublictitle.com`) — WORKING. Statewide (no parish tiering in this tool), $500,000 purchase / $400,000 loan. Result: Owner's Policy Premium **$2,345.20** — byte-identical to the existing FNF entry's own Grand Total for the identical scenario, a striking cross-underwriter (FNF vs. Old Republic) convergence; Lender's Only standalone Premium **$1,429.60**; simultaneous-issue surcharge **$100.00**; Grand Total (simultaneous) **$2,445.20**; Closing-Disclosure-formatted Owner's-Policy line **$1,015.60**. **Crosses LA to the 3-provider calculator-quoted threshold.**

## Calculator harvest addendum (2026-08-20) — richness pass: Stewart Rate Calculator

Richness pass per the 2026-08-19 session's own recommendation (check tracker coverage for already-solved nationwide recipes — Stewart's `/api/SRC/quote`, WFG's Seller Net Sheet API — before hunting a new platform). Prior: 3 calculator-basis providers. This session adds 1 corroborating provider(s), bringing LA to **4 calculator-basis providers**. See LA.json's newest `basis: "calculator"` entries for full itemized figures and methodology.

- **Stewart Title Guaranty Company** (Stewart Rate Calculator, `stewartratecalculator.com`) — matched settlement office **BD Title, LLC**. Owner's Policy Premium **$2,345.20**, Lender's Policy Premium **$1,429.60**, premium-only (no itemized settlement fee returned).
