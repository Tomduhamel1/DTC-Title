...
## Calculator harvest addendum (2026-08-21) — Old Republic's 2nd-tool richness pass: durable NoBot block finally retried and resolved

`Location=IN` on Old Republic's second calculator was logged as a durable NoBot block across the 2026-07-29 and 2026-08-10 sessions, and every session since flagged it as the top candidate to retry with the full session-affinity fix (found on the WV entry, 2026-08-19) — but none actually retried it until this session. **Confirmed the full fix (persistent session, Referer held throughout) resolves it.** Statewide, SIMULTANEOUS LOAN & OWNERS BASIC: Grand Total Policy & Endorsement Premium — Owners+Lenders combined **$1,460.00** (Owner's $1,340.00 + $120.00 simultaneous surcharge), Lenders-only standalone **$380.00**, in the same range as the already-on-file Stewart entry ($1,357.50/$392.50). Also returns a genuine non-premium **TIEFF Policy Fee** (Indiana Title Insurance Enforcement Fund Fee) line item — $5.00 lenders-only / $10.00 combined — a regulatory fee data point not seen on this tool's other state entries. Premium-only otherwise, no settlement-service-fee itemization. 7th corroborating provider — IN rises to **calculator-quoted (7 providers)**. See IN.json's newest `basis: "calculator"` entry for full detail.
